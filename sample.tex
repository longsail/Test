...
\subsection{支持向量机模型}
    ...
    对大规模的数据进行分类时，需要考虑模型训练时间以及模型大小，在领域分类这个任务上，选择使用L1正则的线性核分类器。接下来解释正则以及线性分类器的概念。在机器学习中，使用正则项是为了进行模型选择，主要目的是避免过拟合，常用的正则项有$L_1$以及$L_2$。我们通过下面的两张图来解释$L_1$,$L_2$正则项区别
...
\subsection{incremental learning训练方法}
   ...
   \fbox{\begin{minipage}{32em}
    ...
    3. $\dots \dots$\\
    4.第十个分类器，使用完整的语料集作为训练集同时使用分类器$Y_9$的参数作为第十个分类器参数的初始值，训练出分类器$Y_10$。记录$Y_10$的训练时间以及在测试集上的准确率
    \end{minipage}}
